...
def check_path():
    need1 = load_Eclatorq_path = './Eclatorq/images'
    need2 = save_path = './Eclatorq'
    need3 = save_sop_path = './Eclatorq/sop'
    need4 = save_txt_path = './Eclatorq/sop/labels'
    need5 = temp_yoma_path ='./yoma_data/temp'
    need6 = npy_path = "./Eclatorq/npy/"
    need7 = DATA_DIR = "./Eclatorq/type"
    need8 = yolo_wdir = "./yoma_data/weights"
    need9 = yoma_temp_labels ='./yoma_data/temp/labels'
    need10 = yoma_define_temp ='./yoma_data/define_temp'
    need11 = yoma_define_temp_labels ='./yoma_data/define_temp/labels'

    #新增參數下面要更改
    need_path = []
    for i in range(eval("need11"[4:])):
...
class MyMainForm(QMainWindow, Ui_MainWindow):
    def __init__(self, parent=None):
        ...
        self.trainy_Button.clicked.connect(self.yolo_train)
    ...
    def test_train(self): # type 辨識 
        ttrain = test_train(show = False,epochs = 30) # False True
        ttrain.run()
        QMessageBox.warning(self, "Warning", "Please Re-Open yolo_class")
    def yolo_train(self):
        trainy = yolo_train() # False True
        model_time = trainy.get_model_time()
        print(model_time,type(model_time))
        classes = ['hole','obj']
        category_id_to_name = {0: classes[0], 1: classes[1]}
        yolo_aug =augmentations_yoma(name = 'Eclatorq',augment_num = 2 ,classes = classes,category_id_to_name = category_id_to_name)
    ...
    def classify (self):
        ...
        if self.debug_state == 0 :
            parser = argparse.ArgumentParser()
            parser.add_argument('--weights', nargs='+', type=str, default='best_287.pt', help='model.pt path(s)') # yolov7.pt #20230725best  best_298
            ...
            if img_size_set.isdigit() == True:
                ...
                QMessageBox.warning(self, "Warning", "img_size must be a number ")
        self.showImage()
        ...
        if list_len > 0 :
            ...
            if show_flag=='default' :
                ...
                for self.label_c,self.label_x,self.label_y,self.label_w,self.label_h in self.new_label:
                    ...
                    cv2.rectangle(img, [int(self.xyxy[0]),int(self.xyxy[1])], [int(self.xyxy[2]),int(self.xyxy[3])], (0, 0, 255), 1)
            cv2.imwrite('./yoma_data/define_temp/temp.jpg',img)
            self.show_newImage('./yoma_data/define_temp/temp.jpg')
    ...
    def save(self):
        ...
        if self.frame == []:
            ...
            if ok and save_name:
                ...
                np.savetxt(self.write_sop_label+save_name+".txt", np.asarray(self.save_sop_list), delimiter=" ",fmt='%1.3f')
                    #wirte final txt to  self.write_sop_label


                cv2.imwrite(self.write_sop_img+save_name+'.jpg',img)
                self.show_newImage(self.write_sop_img+save_name+'.jpg')
                self.define_list = []
                self.next()

            # QMessageBox.warning(self, "Warning", "Define The Order")
        elif self.clickmode == 'NewLabel' and len(self.new_label) > 0 and self.mode !='add_type':
            ...
            if ok and Type_Name:
                ...
                type_file_mkdir_name = str('./Eclatorq/type/'+Type_Name)
                ...
                np.savetxt(self.write_sop_label+Type_Name+".txt", np.asarray(self.save_sop_list), delimiter=" ",fmt='%1.3f')
                    #wirte final txt to  self.write_sop_label


                cv2.imwrite(self.write_sop_img+Type_Name+'.jpg',img)
                self.show_newImage(self.write_sop_img+Type_Name+'.jpg')
                self.define_list = []
                self.next()
    ...
    def to_txt (self,jpg_dir):
        ...
        self.open_label ='./yoma_data/define_temp/labels/' #str( self.classify_result_dir) + "/labels/"
        self.img_name = os.path.splitext(os.path.basename(jpg_dir))[0]

        print(self.img_name)
        self.write_sop_img = "./Eclatorq/sop/"
        self.write_sop_label = "./Eclatorq/sop/labels/"

        self.open_txt = self.open_label +self.img_name # D:/pycharm/yolov7/runs/detect/exp25/label/0
        print(self.open_txt)
        self.write_txt = self.write_sop_label +self.img_name # D:/pycharm/yolov7/Eclatorq/sop/label/0
    ...
    def newlabel (self): 
        ...
        if self.label_flag == True:
            ...
            self.clickmode = 'NULL' 
            # self.open_dir_Button.setEnabled(True)
            # np.savetxt(self.write_sop_label+save_name+".txt", np.asarray(self.new_label), delimiter=" ",fmt='%1.6f')

...
import os
from train_mealpy import train_meal
# vram=1024*1#1024*3
...
def model(x_train,y_train,x_test,y_test,class_names,batch_size,parameter):
    ...
    history = model.fit(x_train, y_train,batch_size=batch_size, epochs=30, verbose=1)
...
def model0(problem_dict,term_dict1):
    print('PSO.AIW_PSO')
    model_name.append('PSO')
    model = PSO.AIW_PSO(pop_size=5, c1=2.05, c2=2.05, alpha=0.4)
    g_best = model.solve(problem_dict,termination=term_dict1)
    return g_best
def model1(problem_dict,term_dict1):
    print('ARO.IARO')
    model_name.append('ARO')

    model = ARO.IARO( pop_size=5)
    g_best = model.solve(problem_dict,termination=term_dict1)
    return g_best
# mmodel = ARO.IARO(epoch=20, pop_size=5)
def model2(problem_dict,term_dict1):
    print('TPO.DevTPO')
    model_name.append('TPO')
    model = TPO.DevTPO( pop_size=5, alpha=0.3, beta=50., theta=0.9)
    g_best = model.solve(problem_dict,termination=term_dict1)
    return g_best
def model3(problem_dict,term_dict1):
    print('SA.GaussianSA')
    model_name.append('SA')
    model = SA.GaussianSA( pop_size=5, temp_init = 100, cooling_rate = 0.99, scale = 0.1)
    g_best = model.solve(problem_dict,termination=term_dict1)
    return g_best
def model4(problem_dict,term_dict1):
    print('BA.AdaptiveBA')
    model_name.append('BA')
    model = BA.AdaptiveBA( pop_size=5, loudness_min = 1.0, loudness_max = 2.0)#, pr_min = -2.5, pr_max = 0.85, pf_min = 0.1, pf_max = 10.)
    g_best = model.solve(problem_dict,termination=term_dict1)
    return g_best
...
class mealpy():
    def __init__(self,epoch = 1):
        ...
        self.term_dict1 = {
            ...
            "max_fe": self.epoch+9,  # 100000 number of function evaluation
            "max_time": int(self.epoch*200),  # 10 seconds to run the program
            "max_early_stop": self.epoch  } # 15 epochs if the best objective_function is not getting better we stop the program
        self.problem_dict = {
            "bounds": FloatVar(lb=[1, 1, 1, 1], ub=[257, 257, 257, 257], name="delta"),
    ...
    def objective_function(self,solution):
        ...
        acc = same/len(preds)
        print(acc,"正確率")
        if acc > self.temp_best_acc :
            self.temp_best_acc = acc
        self.temp_best_acc_list.append(self.temp_best_acc)
        self.buf_acc_list.append(acc)
        self.buf_fit_list.append(solution)
        ...
        return error
    def run(self):
        train_m = train_meal(str('meal_05141003'),show =0)

        train_m.run()

        # model_type= ['pso','aro','tpo','sa','ba']
        # if len(model_type)>=1:
        #     history,preds,label= model(x_train,y_train,x_val,y_val,class_names,32,[256.,256,246,234]) #BATCH_SIZE,epochs

        #     for i in range (len(model_type)):
        #         # model = PSO.AIW_PSO(epoch=20, pop_size=5, c1=2.05, c2=2.05, alpha=0.4)
        #         # model = ARO.IARO(epoch=20, pop_size=5)
        #         self.temp_best_acc = 0
        #         self.temp_best_acc_list = []
        #         self.buf_acc_list = []
        #         self.buf_fit_list = []
        #         meal_model = eval("model"+str(i))

        #         g_best = meal_model(self.problem_dict,self.term_dict1)

        #         print(f"Solution: {g_best.solution}, Fitness: {g_best.target.fitness}")
        #         print(len(np.array(self.temp_best_acc_list)),'temp_best_acc_list')
        #         print(len(np.array(self.buf_acc_list)),'buf_acc_list')
        #         print(len(np.array(self.buf_fit_list)),'buf_fit_list')

        #         all_temp_best_acc_list.append(np.array(self.temp_best_acc_list))
        #         all_buf_acc_list.append(np.array(self.buf_acc_list))
        #         all_buf_fit_list.append(np.array(self.buf_fit_list))
        #     fig, ax = plt.subplots()
        ...
        # np.save('./Eclatorq/meal_data/'+str(save_name)+'model_name', model_name)

        # np.save('./Eclatorq/meal_data/'+str(save_name)+'acc', all_temp_best_acc_list)
        # np.save('./Eclatorq/meal_data/'+str(save_name)+'bufacc', all_buf_acc_list)
        # np.save('./Eclatorq/meal_data/'+str(save_name)+'fit', all_buf_fit_list)
        # train_m = train_meal(str(save_name),show =0)
        # print(str(save_name))

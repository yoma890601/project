...
# performance or cause incorrect results. The best thing to do is to ensure that only a single OpenMP runtime is linked into the process, e.g.
# by avoiding static linking of the OpenMP runtime in any library. As an unsafe, unsupported,
# undocumented workaround you can set the environment variable KMP_DUPLICATE_LIB_OK=TRUE to allow the program to continue to execute,
...
def train(hyp, opt, device, tb_writer=None):
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if rank in [-1, 0]:
            ...
            if (not opt.nosave) or (final_epoch and not opt.evolve):  # if save
                ...
                print(best_fitness,fi)
                if best_fitness == fi:
                    torch.save(ckpt, best)# 20231225 save 改這裡
                    torch.save(ckpt, yoma_wdir)# 20231225 save 改這裡
                    #20231228 用舊檔案不會儲存 應該是不夠best
                    #如果不夠好 目前會用yolo v7 還是要讓他抓時間最晚的??
                    #2024 新想法 best 改 new  如果最後有產出上一個改時間 newbest改回best 在train外面用
                    #aug 抓model日期 之前照片aug幾張 之後多張
                    #20240104  下面optimizers後抓上次model. epoch不會繼續 .可以看要開嗎
                    #新想法 可以保存 準確率高於90 或是低於上次1趴內 不然沒有更好得不會儲存
    ...
    if rank in [-1, 0]:
        ...
        for f in last, best,Path(yoma_wdir):
            if f.exists():
...
class yolo_train ():
    ...
    def run(self):
        ...
        parser.add_argument('--weights', type=str, default=self.yoma_wdir, help='initial weights path')
        ...
        # cc = torch.load(weights, map_location=device)['epoch']  # load checkpoint
        # print(cc)      
        # Train
        logger.info(opt)
        if not opt.evolve:

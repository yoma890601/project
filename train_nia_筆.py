...
def plot_confusion_matrix(label_list, preds_list, classes, normalize):
...
def yomatnet(inputs, num_features):
    ...
    # Apply affine transformation to input features
    return layers.Dot(axes=(2, 1))([inputs, feat_T])
def model_or(x_train, y_train, x_test, y_test, class_names, batch_size, parameter,kf_name):
    print("or")
    Dropout_0 = 0.3
...
def model(x_train, y_train, x_test, y_test, class_names, batch_size, parameter,kf_name):
    print("fittt")
    Dropout_0 = 0.3
    Neurons_1 = int(parameter[0])  # Neurons_list[int(parameter[1])] #32
    Neurons_2 = 32
    ...
    model.compile(loss="sparse_categorical_crossentropy", optimizer=optimizer,
                  metrics=["sparse_categorical_accuracy"], )
    history = model.fit(x_train, y_train, batch_size=batch_size, epochs=30, verbose=0)
